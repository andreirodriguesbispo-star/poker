...
// =========================
// Painel de combinações (FIXO E SEGURO)
// =========================
(function () {
  const comboPanel = document.getElementById("comboPanel");
  const comboToggle = document.getElementById("comboToggle");

  if (!comboPanel || !comboToggle) return;

  let aberto = false; // estado controlado manualmente

  comboToggle.onclick = () => {
    aberto = !aberto;

    if (aberto) {
      comboPanel.classList.remove("min");
    } else {
      comboPanel.classList.add("min");
    }
  };
})();
